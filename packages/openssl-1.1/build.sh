...
TERMUX_PKG_MAINTAINER="@Alto1772"
_VERSION=1.1.1s
...
OPENSSL_PREFIX="$TERMUX_PREFIX/opt/openssl1.1"

termux_step_pre_configure() {
	#test -d $TERMUX_PREFIX/include/openssl && mv $TERMUX_PREFIX/include/openssl{,.tmp} || :
	LDFLAGS="-L$OPENSSL_PREFIX/lib -Wl,-rpath=$OPENSSL_PREFIX/lib $LDFLAGS"
}
...
termux_step_configure() {
	...
	install -m755 -d $OPENSSL_PREFIX/lib
...
termux_step_make_install() {
	...
	make -j 1 install_sw MANDIR=$OPENSSL_PREFIX/share/man MANSUFFIX=.ssl

	mkdir -p $TERMUX_PREFIX/etc/tls/

	cp apps/openssl.cnf $TERMUX_PREFIX/etc/tls/openssl.cnf

	install -m755 -d $OPENSSL_PREFIX/include
    ln -s -r -f $OPENSSL_PREFIX/bin/openssl $TERMUX_PREFIX/bin/openssl-1.1
}
